...
class ColorPicker extends StatefulWidget {
  ColorPicker({
    ...
    this.colorPickerWidth: 600.0,
    this.colorPickerheight: 600.0,
  ...
  final double trackPainterHeight = 10.0;
...
class _ColorPickerState extends State<ColorPicker> {
  ...
  Widget getHexValueLable() {
    Color color = HSVColor.fromAHSV(alpha, hue, saturation, value).toColor();
    var hex = color.value.toRadixString(16);
    hex = '#'+hex.substring(2, hex.length);
    return Text(hex,style:TextStyle(color: Colors.black,fontSize: 15,decoration: TextDecoration.none,fontWeight: FontWeight.w400),);
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget _colorPicker(double width) {
      ...
      double height = width * widget.pickerAreaHeightPercent-90;
      ...
      return Container(
        ...
        child: GestureDetector(
          onPanUpdate: (DragUpdateDetails details) {
            ...
            setState(() {
              hue = localOffset.dx.clamp(0.0, width) / width*360;
              saturation = 1 - (localOffset.dy-70).clamp(0.0, height) / height;
          ...
          child: CustomPaint(
            ...
            painter:
            ColorPainter(hue, saturation: saturation, value: value),
          ),
        ),
      );
    }
    ...
    Widget _pickerSlider(double width) {
      return Column(
        children: [ TrackType.alpha].map((TrackType sliderType) {
          if (sliderType == TrackType.alpha && !widget.enableAlpha) {
            return Container();
          }

          double _percent = (sliderType == TrackType.hue) ? hue / 360 :value ;
              ...
              children: <Widget>[
                LayoutId(
                  ...
                  child: ClipRRect(
                    ...
                    child: CustomPaint(
                      painter: TrackPainter(hue,saturation: saturation,alpha: alpha,trackType: sliderType),
                    ),
                  ),
                ),
                LayoutId(
                  ...
                  child: Transform(
                    transform: Matrix4.identity()
                      ..translate((width - 30.0) *(1- _percent) + 10.0),
                    child: CustomPaint(
                        ...
                        onPanUpdate: (DragUpdateDetails details) {
                          ...
                          setState(() {
                            if (sliderType == TrackType.hue) {
                              ...
                              value =1- localOffset.dx.clamp(0.0, box.maxWidth) /
                                  box.maxWidth;
//                              value = alpha;
                              print('value----- $value');
                            }
                        ...
                        Container( height: 50,
                            child:Column(crossAxisAlignment : CrossAxisAlignment.start,children: <Widget>[
                              Align(alignment: Alignment.topLeft,
                                child: widget.enableLabel ? getHexValueLable() : Container(),
                              ),
                              Padding(padding: const EdgeInsets.only(left: 10.0),
                                child: _pickerSlider(box.maxWidth-60),
                              ),
                            ])
...
class ColorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Gradient gradientBq = LinearGradient(
      begin: Alignment.topCenter,
      end: Alignment.bottomCenter,
      colors: [
        Colors.grey.withOpacity(0.0),
    ...
    canvas.drawRect(
        rect,
        Paint()
          ..shader = gradientBq.createShader(rect));
    canvas.drawCircle(
        Offset(size.width * hue/360, size.height * (1 - saturation)),
        size.height * 0.04,
        Paint()
          ..color = Colors.white
          ..strokeWidth = 2
...
class TrackPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final List<Color> alphaColors = [
      HSVColor.fromAHSV(alpha, hue, saturation, 1.0).toColor(),
//    const HSVColor.fromAHSV(1.0, 0.0, 1.0, 1.0).toColor(),
//    const HSVColor.fromAHSV(1.0, 0.0, 1.0, 0.0).toColor(),
      HSVColor.fromAHSV(alpha, hue, saturation, 0.0).toColor(),
